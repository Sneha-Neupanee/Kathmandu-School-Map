...
export default function ComparisonPanel({ schools, referencePoint, onRemove, onClose }) {
    ...
    return (
        <div className="absolute top-20 left-1/2 transform -translate-x-1/2 z-30 bg-white shadow-2xl rounded-xl border border-slate-200 w-11/12 max-w-4xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-4 bg-white overflow-x-auto">
                {!referencePoint && <p className="text-xs text-amber-600 mb-2 font-medium bg-amber-50 p-2 rounded border border-amber-100">Tip: Click anywhere on the map to set a reference point for distance calculation.</p>}
                ...
                <table className="w-full text-left text-sm text-slate-600">
                    <thead className="text-xs text-slate-500 uppercase bg-slate-50 border-y border-slate-200">
                        <tr>
                            ...
                            {schools.map(s => (
                                <th key={s.id} className="px-4 py-3 font-bold text-slate-800 border-l border-slate-100 relative">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        <tr className="hover:bg-slate-50 transition-colors">
                            ...
                            {schools.map(s => (
                                <td key={s.id} className="px-4 py-3 border-l border-slate-100 capitalize">{s.schoolType || 'Unknown'}</td>
                            ))}
                        </tr>
                        <tr className="hover:bg-slate-50 transition-colors">
                            <td className="px-4 py-3 font-medium text-slate-700">Operator</td>
                            {schools.map(s => (
                                <td key={s.id} className="px-4 py-3 border-l border-slate-100">
                        ...
                        <tr className="hover:bg-slate-50 transition-colors">
                            ...
                            {schools.map(s => {
                                if (!referencePoint) return <td key={s.id} className="px-4 py-3 border-l border-slate-100 text-slate-400 italic">-</td>;
                                const dist = turf.distance(turf.point([referencePoint.lng, referencePoint.lat]), turf.point([s.lon, s.lat]), { units: 'kilometers' });
                                return (
                                    <td key={s.id} className="px-4 py-3 border-l border-slate-100 font-semibold text-blue-600">
